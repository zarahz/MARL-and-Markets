...
def make_env(env_key, agents, grid_size=5, setting="", market="", trading_fee=0.05,  max_steps=None, seed=None):
    # markets influence action space and therefore needed in the environment directly
    competitive = "competitive" in setting
    env = gym.make(id=env_key, agents=agents, size=grid_size, competitive=competitive,
    ...
    env = MultiagentWrapper(env, setting)
    env.seed(seed)
    return env
